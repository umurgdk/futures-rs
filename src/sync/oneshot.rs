...
use future::{lazy, Lazy, Spawn, IntoFuture};
use lock::Lock;
use task::{self, Task};
...
/// `Spawn` instance provided to that function.
///
...
pub struct Spawned<F: Future> {
    future: F,
    tx: Arc<SpawnInner<Result<F::Item, F::Error>>>,
}

/// Spawns a future `f` onto the instance of `Spawn` provided, `s`, returning a
/// handle representing the completion of the future.
///
/// The `SpawnHandle` returned is a future that is a proxy for `f` itself. When
/// `f` completes on `s` then the `SpawnHandle` will itself be resolved.
/// Internally `SpawnHandle` contains a `oneshot` channel and is thus safe to
/// send across threads.
///
/// The future `f` will be canceled if the `SpawnHandle` is dropped. If this is
...
/// spawn the future `f` provided.
///
/// If the provided instance of `Spawn` does not actually run `f` to completion,
/// then the returned handle may panic when polled. Typically this is not a
/// problem, though, as most instances of `Spawn` will run futures to
/// completion.
pub fn spawn<F, S>(f: F, s: &S) -> SpawnHandle<F::Item, F::Error>
    where F: Future,
          S: Spawn<Spawned<F>>,
{
    let data = Arc::new(SpawnInner {
        inner: Inner::new(),
...
pub fn spawn_fn<F, R, S>(f: F, s: &S) -> SpawnHandle<R::Item, R::Error>
    where F: FnOnce() -> R,
          R: IntoFuture,
          S: Spawn<Spawned<Lazy<F, R>>>,
{
    spawn(lazy(f), s)
}
...
impl<F: Future + fmt::Debug> fmt::Debug for Spawned<F> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
